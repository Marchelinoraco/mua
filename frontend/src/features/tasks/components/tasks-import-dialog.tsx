  ...
  const formSchema = z.object({
    file: z
      ...
      .refine((files) => files.length > 0, {
        message: t('importDialog.validation.fileRequired'),
      })
      .refine(
        (files) => ['text/csv'].includes(files?.[0]?.type),
        t('importDialog.validation.csvOnly')
      ),
  })
    ...
    >
      <DialogContent className='gap-2 sm:max-w-sm'>
            ...
            <FormField
              ...
              render={() => (
                <FormItem className='my-2'>
                  <FormLabel>{t('importDialog.fileLabel')}</FormLabel>
                  <FormControl>
                    <Input
                      type='file'
                      accept='text/csv'
        ...
        <DialogFooter className='gap-2'>
          <DialogClose asChild>
            <Button variant='outline'>{t('importDialog.close')}</Button>
          </DialogClose>
