...
#### Ditambahkan
- **`backend/src/tenant/tenant.service.spec.ts`** (baru, 8 kasus) — `TenantService` sebelumnya **tidak punya file spec sama sekali**, padahal milestone ini menambah cabang logika baru (`assertRegencyExists` — 400 `BadRequestException` bila `regencyId` tidak merujuk Regency manapun, bukan 500 FK constraint) yang nol tercakup. Menutupi: `getMyTenant` (join Regency→kota tampilan, fallback teks lama, `NotFoundException`), `updateMyTenant` (400 saat `regencyId` invalid — update tidak dieksekusi sama sekali; sukses saat valid; tidak menyentuh tabel Regency bila `regencyId` tidak dikirim), `checkSlug` (format invalid/tersedia/saran slug).
- **`backend/src/auth/auth.service.spec.ts`** (baru, 9 kasus) — `AuthService.register()` mengimplementasikan F01 (onboarding trial, alur kritis wajib diuji) dan juga **tidak punya spec sama sekali** sebelum sesi ini. Menutupi cabang baru milestone wilayah: 400 saat `regencyId` invalid (sebelum transaksi dimulai — `$transaction` dipastikan tidak terpanggil), sukses registrasi **tanpa** `regencyId` (opsional by design — tenant tetap dibuat, Theme default + Subscription `TRIALING` tetap jalan), sukses **dengan** `regencyId` valid (kota ter-derive dari join Regency); plus guard existing (`ConflictException` email/slug terdaftar, `InternalServerErrorException` bila tidak ada Plan aktif) dan `login()` (password salah, tenant kota ter-mapping). `bcrypt` di-mock (`jest.mock('bcrypt')`) supaya deterministik & cepat.
- Backend: **170 → 187 test (16 → 18 suite), tetap 100% hijau** (`npm test`).

#### Diperiksa
- `cd backend && npm test` — **187/187 lulus (18 suite)**, klaim awal 170/170 (16 suite) terkonfirmasi akurat sebelum penambahan 2 spec di atas.
- Migrasi `backend/prisma/migrations/20260722180759_add_wilayah_reference/migration.sql` dibaca baris per baris — **benar-benar additive**: `ALTER TABLE "Tenant" ADD COLUMN "regencyId"` (nullable, tanpa `DROP COLUMN kota`), `CREATE TABLE Province/Regency`, FK `Tenant.regencyId → Regency` dengan `ON DELETE SET NULL` (tenant tidak pernah gagal/hilang walau Regency dihapus). Nol perubahan destruktif ke data existing.
- `grep -rn "\.kota\b" backend/src` (di luar file `.spec.ts`) — dikonfirmasi **hanya 3 pemakai** (`TenantService`, `AuthService`, `StorefrontService`), ketiganya konsisten memakai `toTenantKotaDisplay`/`toKotaDisplayName` dari `common/wilayah/tenant-kota.util.ts` (fallback `regency?.nama ?? kota ?? null`) — tidak ada tempat yang membaca `tenant.kota` mentah tanpa fallback.
- Idempotensi `backend/prisma/seed-wilayah.ts` — dikonfirmasi via baca kode (`upsert` by `kode` unik, bukan `create`) **dan** dijalankan ulang nyata ke Neon **dev**: 38 provinsi + 514 kabupaten/kota sebelum & sesudah re-run, **nol duplikat** (`GROUP BY kode HAVING COUNT(*) > 1` = 0 baris untuk keduanya).
- Endpoint edge case via `curl` ke `dist/src/main` (build terkonfirmasi lebih baru dari seluruh `src/*.ts`, jadi merefleksikan kode working-tree saat ini) + Neon dev: `GET /api/wilayah/regencies` tanpa `provinceId` → **400** "Parameter provinceId wajib diisi." (bukan 500/array kosong); `?provinceId=id-tidak-valid` → **200 `[]`** (bukan error); `POST /api/auth/register` dengan `regencyId` tidak valid → **400** "regencyId tidak valid." (bukan 500 FK); `POST /api/auth/register` **tanpa** `regencyId` → **201**, tenant dibuat dengan `regencyId:null, kota:null` (dikonfirmasi opsional by design sesuai `RegisterDto`, tidak mewajibkan kota). Data uji dibersihkan kembali dari Neon dev setelah verifikasi.
- Regresi `GET /api/s/:slug`: dicek terhadap 2 tenant existing di Neon dev — satu dengan `kota` teks bebas lama & `regencyId` null ("Jakarta") dan satu dengan `regencyId` ter-mapping (join → "Kota Manado") — keduanya mengembalikan kontrak **identik** (`kota: string | null`, tanpa field `provinceId`/`provinsi` bocor ke publik; field-field tambahan itu sengaja hanya ada di DTO privat/terautentikasi `TenantResponseDto`/`AuthResponseDto`/`MeResponseDto`, dikonfirmasi lewat pembacaan `StorefrontProfileResponseDto`). Nol regresi kontrak publik.
- `cd frontend && npx vitest run --browser.headless` (full suite, dijalankan 2×, hasil identik keduanya) — **25/38 file lulus, 206/270 test lulus**. 13 file gagal (64 test), daftar lengkapnya: `components/config-drawer.test.tsx`, `components/sign-out-dialog.test.tsx`, `context/search-provider.test.tsx`, `features/auth/forgot-password/…`, `features/auth/otp/…`, `features/auth/sign-in/…`, `features/tasks/components/{tasks-import-dialog,tasks-multi-delete-dialog,tasks-mutate-drawer}.test.tsx`, `features/users/components/{users-action-dialog,users-delete-dialog,users-invite-dialog,users-multi-delete-dialog}.test.tsx` — **cocok 100% dengan baseline pre-existing yang sudah didokumentasikan berkali-kali sebelumnya** (pola timeout ~15s per interaksi Playwright), **nol** file terkait wilayah/sign-up/storefront/booking di dalamnya. Angka ini **sedikit lebih baik** dari baseline terakhir (14 file/68 test gagal dari 36 file/261 test) — konsisten dengan nol regresi (bertambah 2 file passing baru `features/wilayah/*`, dan `sign-up-form.test.tsx` yang dulu gagal total kini lulus). Scoped run terisolasi untuk fokus wajib milestone ini: `npx vitest run src/features/wilayah src/features/auth/sign-up --browser.headless` → **3/3 file, 13/13 test lulus** — mengkonfirmasi ulang klaim frontend-engineer secara independen.

#### Verdict
- **MILESTONE SIAP COMMIT.** Nol regresi terdeteksi, migrasi & seed aman/idempoten, fallback kota konsisten di semua pembaca, edge case endpoint sesuai ekspektasi (400 untuk input invalid, bukan 500), kontrak storefront publik tidak berubah. Satu gap coverage nyata ditemukan (`AuthService`/`TenantService` nol spec) — **ditutup** dengan menambah 17 test baru (lihat di atas), bukan blocker.

### 2026-07-23 — Frontend: dropdown Provinsi → Kota/Kabupaten di sign-up (menggantikan input kota teks bebas)

#### Ditambahkan
- **`frontend/src/features/wilayah/`** (modul baru, reusable) — `data/types.ts` (`Province`, `Regency`, cocok kontrak `WilayahProvinceDto`/`WilayahRegencyDto`), `hooks/use-provinces.ts` (`GET /api/wilayah/provinces`, `staleTime: Infinity` — data referensi statis), `hooks/use-regencies.ts` (`GET /api/wilayah/regencies?provinceId=`, `enabled` hanya setelah provinsi dipilih supaya tidak memicu 400 dari backend). Ditempatkan sebagai modul lintas-fitur (bukan di dalam `features/auth`) supaya bisa dipakai ulang di form manapun yang butuh field wilayah nanti (mis. onboarding, tenant settings — belum ada saat ini, lihat catatan gap di bawah).
- **`frontend/src/features/wilayah/hooks/use-provinces.test.tsx`** (2 kasus) + **`use-regencies.test.tsx`** (4 kasus) — unit test hook dengan `QueryClientProvider` wrapper & `@/lib/api` di-mock; menutupi fetch sukses, error state, `enabled:false` saat `provinceId` kosong, dan refetch saat `provinceId` berganti.

#### Diubah
- **`frontend/src/features/auth/sign-up/components/sign-up-form.tsx`** — field "Kota" (input teks bebas) di step 2 (Profil Bisnis) diganti dua `Select` berkaskade: Provinsi → Kota/Kabupaten (pakai `SelectDropdown` yang sudah ada, pola sama seperti `users-action-dialog.tsx`/`tasks-mutate-drawer.tsx` — bukan pola baru). Provinsi memuat `useProvinces()` sekali (cached); Kota/Kabupaten disabled sampai provinsi dipilih, memuat `useRegencies(provinceId)`, dan otomatis di-reset (`form.setValue('regencyId', '')`) tiap kali provinsi diganti. Kedua dropdown punya state loading (`isPending` bawaan `SelectDropdown`, trigger disabled saat fetch) dan error (pesan + tombol "Coba lagi" yang memanggil `refetch()`). Skema Zod step 2 menambah `provinceId` (validasi UI saja, wajib dipilih supaya kota bisa difilter) dan `regencyId` (wajib dipilih) menggantikan `kota`. Payload yang dikirim ke `POST /auth/register` sekarang `regencyId` — **`provinceId` sengaja TIDAK ikut dikirim** (backend hanya butuh `regencyId`, provinsi bisa di-derive dari relasi Regency→Province di backend). Value `provinceId` dibaca via `useWatch({control, name})` (bukan `form.watch()`) supaya tidak memicu warning React Compiler "incompatible library" saat nilainya diteruskan ke hook lain (`useRegencies`).
- **`frontend/src/stores/auth-store.ts`** — tipe `AuthTenant` menambah `regencyId?: string | null`, `provinceId?: string | null`, `provinsi?: string | null`; `kota` diubah dari `string` wajib menjadi `string | null` opsional, menyesuaikan `AuthResponseDto`/`MeResponseDto`/`TenantResponseDto` backend yang baru (field ini bisa `null` bila tenant belum ter-mapping ke Regency).
- **`frontend/src/locales/{id,en}/auth.json`** — namespace `signUp`: tambah `provinsi`, `provinsiPlaceholder`, `provinsiError`, `kotaHintPilihProvinsi`, `kotaError`, `retry`; `kota` (label) diubah dari "Kota" → "Kota/Kabupaten" (EN: "City" → "City/Regency"); `kotaPlaceholder` diubah dari contoh teks bebas ("Contoh: Jakarta") menjadi placeholder dropdown ("Pilih kota/kabupaten"); `validation.provinsiRequired` ditambah, `validation.kotaRequired` disesuaikan teksnya untuk konteks dropdown (catatan: seluruh key `signUp.validation.*` sudah lama tidak benar-benar dipakai runtime — pesan Zod di komponen memakai string literal langsung, bukan `t()` — dipertahankan apa adanya sebagai dokumentasi, tidak diperbaiki karena di luar lingkup tugas ini).
- **`frontend/src/features/auth/sign-up/components/sign-up-form.test.tsx`** — ditulis ulang total. File lama sudah gagal total sebelum sesi ini (baseline dikonfirmasi: memakai label Inggris usang "Create Account" yang tidak cocok dengan komponen berbahasa Indonesia manapun — bug pre-existing, bukan regresi dari task ini). Versi baru: dibungkus `QueryClientProvider` (dibutuhkan `useProvinces`/`useRegencies`), mengunci locale test ke `id` via `i18n.changeLanguage('id')` (lihat catatan gap di bawah), dan menambah 4 skenario baru untuk dropdown wilayah (render+disabled, pilih provinsi→kota terisi→submit `regencyId` saja, ganti provinsi mereset kota, validasi wajib pilih). 7/7 lulus.

#### Diperiksa
- `npx tsc -b` — bersih, nol error. `npm run build` — sukses (warning ukuran chunk >500kB sudah ada sebelumnya, tidak terkait perubahan ini). `npx eslint` pada seluruh file yang disentuh (termasuk `sign-up-form.tsx`, modul `wilayah/`, `auth-store.ts`) — nol error, nol warning (2 isu ditemukan & diperbaiki selama proses: `no-duplicate-imports` pada `@/stores/auth-store` — pre-existing di file ini sebelum diedit, digabung jadi satu import dengan inline `type` modifier sesuai `fixStyle: 'inline-type-imports'` di `eslint.config.js`; dan warning React Compiler "incompatible library" dari `form.watch()` yang nilainya diteruskan ke hook lain — diganti `useWatch({control,name})`).
- `cd frontend && npx vitest run src/features/wilayah src/features/auth/sign-up --browser.headless` — **13/13 lulus** (2 file hook baru + 1 file komponen ditulis ulang).
- E2E manual via `curl` terhadap backend dev nyata (Neon dev, 514 kabupaten/kota ter-seed, sesuai instruksi — **bukan Playwright**): `GET /api/wilayah/provinces` (38 provinsi, urut asc, terkonfirmasi), `GET /api/wilayah/regencies?provinceId=<DKI Jakarta>` (6 kota/kabupaten Adm. Jakarta, terkonfirmasi), `GET /api/wilayah/regencies` tanpa param → 400 "Parameter provinceId wajib diisi." (terkonfirmasi), `POST /auth/register` dengan `regencyId` valid (Kota Adm. Jakarta Selatan) → 201, response `tenant` berisi `regencyId`, `kota:"Kota Adm. Jakarta Selatan"`, `provinceId`, `provinsi:"DKI Jakarta"` persis sesuai tipe `AuthTenant` yang diperbarui; `POST /auth/register` dengan `regencyId` acak/tidak valid → 400 "regencyId tidak valid." (bukan 500), akan ditangani `handleServerError` yang sudah ada di `onSubmit`. **Catatan:** satu baris tenant uji (`test-wilayah-*@contoh.com`) tertinggal di Neon **dev** dari verifikasi ini — data uji, bukan produksi, konsisten dengan tenant uji lain yang sudah ada di dev per catatan sesi backend sebelumnya.

#### Gap / temuan untuk tech-lead (tidak diperbaiki sesi ini, di luar lingkup)
- **`frontend/src/features/onboarding/`** — dicek isinya (`index.tsx` + `components/`), **tidak ada field kota terpisah dari sign-up** di alur onboarding (step 1 hanya menampilkan `namaBisnis` read-only, step 2 payment profile, step 3 stub layanan) — tidak ada yang perlu diubah, sesuai dugaan di instruksi tugas.
- **Tidak ada halaman "tenant settings" di FE yang mengedit `namaBisnis`/kota setelah sign-up**, walau backend sudah punya `GET`/`PATCH /tenants/me` (`TenantController`, lihat `tenant-response.dto.ts`/`update-tenant.dto.ts`) siap pakai. Artinya field kota tenant saat ini **hanya bisa diisi sekali saat registrasi** — tidak bisa diubah lagi dari UI manapun setelahnya. Ini gap produk, bukan sesuatu yang diminta di task ini (task hanya minta upgrade field kota yang SUDAH ADA jadi dropdown) — perlu keputusan tech-lead apakah halaman "Profil Bisnis" di `settings/` perlu dibuat sebagai fast-follow (kalau dibuat, hook `useProvinces`/`useRegencies` dari `features/wilayah/` sudah siap dipakai ulang di sana, termasuk pre-fill dari `regencyId`/`provinceId` yang sudah dikembalikan `TenantResponseDto`).
- **Gap test infrastruktur pre-existing (bukan dari task ini, tapi ditemukan saat mengerjakannya)**: `src/features/auth/sign-in/components/user-auth-form.test.tsx`, `forgot-password-form.test.tsx`, `otp-form.test.tsx` (dan `sign-up-form.test.tsx` sebelum diperbaiki sesi ini) **gagal total** bila dijalankan — dua sebab tumpang tindih: (1) tidak mengimpor `@/lib/i18n` secara eksplisit sehingga `useTranslation` melempar `NO_I18NEXT_INSTANCE`, dan (2) `i18next-browser-languagedetector` meresolusi locale test Playwright headless ke `en` (dari `navigator.language`), bukan `id` (fallback), jadi assertion yang menebak teks Bahasa Indonesia gagal tak-deterministik. Pola perbaikan sudah dicontohkan di `sign-up-form.test.tsx` versi baru (`import i18n from '@/lib/i18n'` + `i18n.changeLanguage('id')` di `beforeEach`) — tiga file lain **belum** diperbaiki karena di luar lingkup task kota/wilayah ini; direkomendasikan sebagai fast-follow test-debt ke `qa-testing`.

### 2026-07-23 — Data referensi wilayah Indonesia (Provinsi/Kabupaten-Kota) — skema, migrasi, seed

#### Ditambahkan
- **`backend/prisma/schema.prisma`** — model baru `Province` (`id, kode(unik), nama`) dan `Regency` (`id, kode(unik), nama, provinceId`, index `(provinceId)`, `onDelete: Cascade` ke Province) [global], read-only, diimpor sekali via seed. `Tenant.regencyId` (String?, FK opsional ke Regency, `onDelete: SetNull`) ditambahkan sebagai rujukan wilayah terstruktur pengganti `Tenant.kota` teks bebas. Index tambahan `Tenant(regencyId)`.
- **`backend/prisma/migrations/20260722180759_add_wilayah_reference/`** — migrasi additive (create table Province/Regency, alter table Tenant tambah kolom `regencyId` + FK) dijalankan ke Neon **dev**; **kolom `kota` lama TIDAK dihapus** (dipertahankan sebagai fallback baca sampai tech-lead konfirmasi migrasi data aman).
- **`backend/prisma/seed-wilayah.ts`** + **`backend/prisma/seed-data/wilayah-indonesia.json`** (vendored, snapshot statis 22 Jul 2026) — seed sekali-jalan (`npx ts-node prisma/seed-wilayah.ts`, TIDAK dipanggil dari runtime). Sumber: API pihak ketiga tanpa SLA `api-wilayah-indonesia-v1.vercel.app` (`GET /api/provinces`, `GET /api/cities?province_code=`) — endpoint kabupaten/kota dikonfirmasi lewat percobaan curl (`/api/regencies/*` 404, pola yang benar = `/api/cities?province_code=`). Divendor sebagai JSON statis (bukan fetch live di seed) supaya seed idempoten & tidak bergantung API pihak ketiga saat dijalankan ulang. Dijalankan ke Neon **dev**: **38 provinsi + 514 kabupaten/kota** ter-upsert (cocok dengan jumlah resmi Kemendagri).
- **`backend/prisma/migrate-tenant-kota.ts`** — skrip migrasi data sekali-jalan `Tenant.kota` (teks bebas) → `regencyId`, best-effort name matching (exact match, lalu strip prefix "Kota"/"Kabupaten"/"Kab."). Dijalankan ke Neon **dev**: dari 2 tenant lama berisi `kota`, **1 ter-mapping** (`"Manado"` → `Kota Manado`), **1 tidak ter-mapping** (`"Jakarta"` — ambigu, bukan nama kabupaten/kota valid, tetap `regencyId=null` & `kota` lama utuh) — dilaporkan eksplisit di output skrip untuk review manual.
- **`backend/src/wilayah/`** (modul baru) — `WilayahController` + `WilayahService`, 2 endpoint publik tanpa auth, throttle 60/menit: `GET /api/wilayah/provinces` → `[{id,kode,nama}]` urut nama asc; `GET /api/wilayah/regencies?provinceId=` → `[{id,kode,nama}]` urut nama asc, 400 bila `provinceId` tidak diisi. Diekspor dari `AppModule`.
- **`backend/src/common/wilayah/tenant-kota.util.ts`** (+ spec) — util pure-function pemetaan "kota tampilan" tenant (`toTenantKotaDisplay`, `toKotaDisplayName`): utamakan join `Regency.nama` (+ `Province.nama`), fallback ke `kota` teks lama bila tenant belum ter-mapping. Dipakai `TenantService`, `AuthService`, `StorefrontService` supaya logika fallback konsisten di satu tempat.
- **`backend/src/wilayah/wilayah.service.spec.ts`** — unit test `WilayahService` (query shape orderBy nama asc, filter provinceId, array kosong bila tidak ada hasil).

#### Diubah
- **`backend/src/auth/dto/register.dto.ts`**, **`backend/src/tenant/dto/update-tenant.dto.ts`** — field `kota?: string` (teks bebas) diganti `regencyId?: string` (FK ke Regency; divalidasi keberadaannya di service, 400 bila tidak valid — bukan 500 FK constraint).
- **`backend/src/tenant/dto/tenant-response.dto.ts`**, **`backend/src/auth/dto/me-response.dto.ts`**, **`backend/src/auth/dto/auth-response.dto.ts`** — field `tenant.kota` dipertahankan (nama tampilan, kini sumbernya join `Regency.nama` dengan fallback teks lama) + field baru `regencyId`, `provinceId`, `provinsi` untuk pre-fill dropdown cascading Provinsi→Kab/Kota di FE tanpa round-trip tambahan.
- **`backend/src/tenant/tenant.service.ts`**, **`backend/src/auth/auth.service.ts`** — `getMyTenant`/`updateMyTenant` (Tenant) dan `register`/`getMe`/`login` (Auth) kini select+join `Regency`/`Province`, mapping via `toTenantKotaDisplay`; `updateMyTenant`/`register` validasi `regencyId` (400 `BadRequestException` bila tidak ada baris Regency yang cocok) sebelum menulis.
- **`backend/src/storefront/storefront.service.ts`** — `STOREFRONT_TENANT_SELECT` join `regency: {select:{nama}}`; field publik `kota` (kontrak tidak berubah, tetap `string | null`) kini sumbernya `toKotaDisplayName` (join, fallback teks lama) — **nol perubahan kontrak untuk FE storefront publik**.
- **`docs/data-model.md`** — entri `Tenant` diperbarui (`kota?` deprecated + `regency_id?`), seksi baru "Wilayah *(referensi, [global], read-only)*" (Province/Regency), diagram ERD tambah relasi `TENANT }o--o| REGENCY` & `PROVINCE ||--o{ REGENCY`.

#### Diperiksa
- Neon **dev** dicek sebelum migrasi data: `SELECT COUNT(*) FROM "Tenant" WHERE kota IS NOT NULL` → 2 dari 6 tenant total (data uji QA, bukan data produksi nyata).
- `cd backend && npm run build` — hijau, nol type error. `npm test` — **170/170 lulus** (16 suite, naik dari 159 — 11 test baru: `tenant-kota.util.spec.ts` 6, `wilayah.service.spec.ts` 3, 2 kasus baru di `storefront.service.spec.ts` untuk join vs fallback kota).
- Tidak ada migrasi/seed yang dijalankan ke Neon **production** — hanya branch **dev**, sesuai instruksi.

#### Belum Dikerjakan (perlu delegasi FE terpisah)
- Frontend TIDAK disentuh sesi ini. Tempat yang menggunakan `Tenant.kota` dan perlu diperbarui menyesuaikan kontrak baru (`regencyId`/`kota`/`provinceId`/`provinsi` + dropdown wilayah baru): `frontend/src/features/auth/sign-up/components/sign-up-form.tsx` (form registrasi — kota teks bebas → dropdown Provinsi/Kab-Kota via `GET /api/wilayah/*`), `frontend/src/stores/auth-store.ts` (shape tenant di state), `frontend/src/features/storefront-public/components/storefront-hero.tsx` + `frontend/src/features/storefront-public/data/types.ts` (tampilan publik — kontrak `kota: string|null` TIDAK berubah, aman tanpa perubahan FE, opsional), `frontend/src/features/onboarding/index.tsx` (alur onboarding, cek pemakaian namaBisnis/kota), `frontend/src/locales/{id,en}/auth.json` & `storefront.json` (label field bila UI dropdown diubah).
